...
function removeSlideObject(slide: Slide, objectId: string): Slide {
    return { ...slide, content: slide.content.filter(obj => obj.id !== objectId) };
}
...
function setObjectPositionCoordinates(slideObject: SlideObject, position: {x: number; y: number} ): SlideObject {
    return { ...slideObject, position: {...slideObject.position, ...position} } as SlideObject;
}

// изменение размера объекта
function setObjectPositionSize(slideObject: SlideObject, size: {width: number; height: number} ): SlideObject {
    return { ...slideObject, position: { ...slideObject.position, width: size.width, height:  size.height } } as SlideObject;
}

// Изменение размера шрифта для текста
function setTextSize(slide: Slide, textId: string, size: number): Slide {
    const content = slide.content.map(obj => obj.type === 'text' && obj.id === textId
